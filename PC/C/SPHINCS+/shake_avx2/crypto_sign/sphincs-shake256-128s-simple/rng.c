...
int randombytes(unsigned char *x, unsigned long long xlen) {
    ...
    int i = 0;
    if (!state_seeded) return RNG_BAD_REQ_LEN;
    while (xlen > 0) {
        for (int j = 15; j >= 0; j--) {
        ...
        if (xlen > 15) { memcpy(x + i, block, 16); i += 16; xlen -= 16; }
        else { memcpy(x + i, block, xlen); xlen = 0; }
    }
