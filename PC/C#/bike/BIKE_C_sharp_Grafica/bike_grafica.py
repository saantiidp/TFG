...
YMIN, YMAX = 0.05, 300.0 # usado si RANGE_MODE == "manual"
...
FILE_PATTERNS = {
    "BIKE-128": ["bike128*performance*.csv", "bike_128*performance*.csv", "bike128*.csv"],
    "BIKE-192": ["bike192*performance*.csv", "bike_192*performance*.csv", "bike192*.csv"],
    "BIKE-256": ["bike256*performance*.csv", "bike_256*performance*.csv", "bike256*.csv"],
}
...
def load_bike_csv(path: str, tag: str) -> pd.DataFrame:
...
for tag, patterns in FILE_PATTERNS.items():
    ...
    for f in files:
        try:
            frames.append(load_bike_csv(f, tag))
            print(f"[OK] {tag}: {f}")
        except Exception as e:
            print(f"[X]  {tag}: {f} -> {e}")

if not frames:
    raise SystemExit("No se pudo cargar ningún CSV de BIKE.")

df = pd.concat(frames, ignore_index=True)

# -------- Plot por operación --------
versions = ["BIKE-128","BIKE-192","BIKE-256"]
...
plt.title("BIKE — Comparación por operación (128/192/256)")
...
plt.tight_layout()
plt.savefig("bike_boxplot_por_operacion.png", dpi=150)
# plt.show()
